...
function Login({ onLogin }) {
  ...
  const handleClientLogin = async (e) => {
    ...
    try {
      ...
      localStorage.setItem('authToken', discordId);
      onLogin(userData, false, true, discordId);
    } catch (error) {
      setError('Failed to verify Discord ID. Please try again.');
      console.error(error);
    }
  };
  ...
  const handleDiscordLogin = async () => {
    try {
      const response = await axios.get(`${API_URL}/api/auth/discord`);
      ...
      setError('Discord OAuth not available. Please use manual login.');
      console.error(error);
    }
  };
    ...
    <div className="login-container">
      <div className="login-card">
        ...
        {discordOAuthEnabled && (
          <div className="discord-oauth-section">
            <button
              ...
              className="btn btn-discord login-btn"
            >
        ...
        <div className="login-tabs">
          ...
          <button
            className={`tab ${mode === 'developer' ? 'active' : ''}`}
        ...
        <form onSubmit={mode === 'client' ? handleClientLogin : handleDeveloperLogin}>
          {mode === 'client' ? (
